    ...
    */
    public class AVL_1
    {
        ...
        */
        ...
        {
            //Console.WriteLine(Input);
            if (modOutput == 1) DebuggingWindow_1.PrintTree(Input.ToString() + "\n");
        ...
        public class Key_1
        {
            ...
            public string fio, login, gend, adress;

            // Конструктор
            public Key_1(string fio_v, string login_v, string gend_v, string adress_v)
            {
                next = null;
                fio = fio_v;
                login = login_v; // name
        ...
        public class Node
        {
            ...
            public Node(string fio_v, string login_v, string gend_v, string adress_v)
            {
                left = null;
                right = null;
                height = 1;
                key = new Key_1(fio_v, login_v, gend_v, adress_v);
            }
        };
        ...
        bool warningWindowForStringIsVisible = false; 
        // Показ окна, которое предупреждает о том, что сравнение просиходит не как числа, а как строки, при вводе числа больше 10
        ...
        void CheckAttentionAddFromTree(string Adress)
        {
            ...
            {
                if ((adrInp >= 10) && (warningWindowForStringIsVisible == false))
                {
                    ...
                    warningWindowForStringIsVisible = true;
                }
            }
        }

        // Основная процедура вставки
        public bool MainAdd(string Fio, string Login, string Gend, string Adress)
        {
            CheckAttentionAddFromTree(Adress);
            ...
            if (root == null)
            {
                root = new Node(Fio, Login, Gend, Adress);
            }
            else
            {
                root = insertNode(root, Fio, Login, Gend, Adress);
            }
            return true;
        }

        // Вставка элемента в дерево
        public Node insertNode(Node node, string Fio, string Login, string Gend, string Adress)
        {
            if (node == null)
            {
                Node newNode = new Node(Fio, Login, Gend, Adress);
                return (newNode);
            }

            Node Sear = InsideSearch(root, Adress);

            if (Sear == null)
            {
                if (!a_big_b(Adress, node.key.adress))
                    node.left = insertNode(node.left, Fio, Login, Gend, Adress);
                else if (a_big_b(Adress, node.key.adress))
                    node.right = insertNode(node.right, Fio, Login, Gend, Adress);
                else
                    return node;
            }
            else
            {
                Key_1 vertex = new Key_1(Fio, Login, Gend, Adress);
            ...
            if (balanceFactor > 1)
            {
                if (!a_big_b(Adress, node.left.key.adress))
                {
                    return rightRotate(node);
                }
                else if (a_big_b(Adress, node.left.key.adress))
                {
            ...
            if (balanceFactor < -1)
            {
                if (a_big_b(Adress, node.right.key.adress))
                {
                    return leftRotate(node);
                }
                else if (!a_big_b(Adress, node.right.key.adress))
                {
        ...
        public void MainDel(string Adress)
        {
            //if (TestingNumberKeysMod == true)
            //{
            //    Adress = NumbersToLiterals(Adress);
            ...
            root = deleteNode(root, Adress, false);
        }

        // Удаление по ключу
        public Node deleteNode(Node root, string Adress, bool isDontReturnDubleNode)
        {
            if (root == null)
                return root;

            if (Adress == root.key.adress)
            {
                if ((root.key.next == null) || (isDontReturnDubleNode == true))
                {
                    ...
                    else
                    {
                        ...
                        root.key = temp.key;
                        root.right = deleteNode(root.right, temp.key.adress, true);
                    }
                }
                else
                {
                ...
                root.left = deleteNode(root.left, Adress, isDontReturnDubleNode);
            else //if (a_big_b(Adress, root.key.adress))
                root.right = deleteNode(root.right, Adress, isDontReturnDubleNode);
        ...
        // Поиск по ключу
        public bool SearchAdressOnly(Node node, string Adress)
        {
            if ((node != null) && (node.key != null))
            {
                if (Adress == node.key.adress)
                {
                    ResSearch = node;
                    return true;
                }
                else if (a_big_b(Adress, node.key.adress))
                    SearchAdressOnly(node.right, Adress);
                else
                    SearchAdressOnly(node.left, Adress);
            }
        ...
        public Node InsideSearch(Node root1, string Adress)
        {
            ResSearch = null;

            SearchAdressOnly(root1, Adress);

            return (ResSearch);
        }

        // Основная процедура поиска
        public Node MainSearch(string Adress)
        {
            ResSearch = null;
            ...
            SearchAdressOnly(this.root, Adress);
        ...
        public void CoutNode(Key_1 p, int mod)
        {
            ...
            if (mod == 0) coutnn(str);
            else coutnn("" + p.fio + ", " + p.login
                + ", " + p.gend + ", " + str);
        }

        // Обрабатываю вывод, если в одной вершине несколько дублей
        ...
        //
        public void PrintNode1(Node p, string Login)
        {
            if (p != null)
            {
                if (p.key.next == null)
                {
                    if (p.key.login == Login)
                        isLogFound = true;                
                }
                else // Если не одна запись
                {
        ...
        public void LKPtoLog(Node n, string Login)
        {
            if ((n != null) && (isLogFound == false))
            {
                LKPtoLog(n.left, Login);
                PrintNode1(n, Login);
                //if (n.key.login == Login)
                //    isLogFound = true;
                LKPtoLog(n.right, Login);
            }
        }

        // Если нашли запись с таким логином в нашем дереве - возвращаем true
        public bool SearchLogin(string Login)
        {
            isLogFound = false;
            LKPtoLog(root, Login);
            
            if (isLogFound == true) return true;
            else return false;
        }

        public void DebugPrint()
        {
            int mod = DebuggingWindow_1.ModOutput;
            modOutput = 1;
            DebuggingWindow_1.AllDel();
            DebuggingWindow_1.Mod = true;
            if (root == null)
            {
                cout("[Дерево пусто]");
            }
